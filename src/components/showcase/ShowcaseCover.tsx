...
const ShowcaseCover = ({ project, compact = false }: { project: ShowcaseProject; compact?: boolean }) => (
  <div className={`relative flex h-full min-h-[240px] overflow-hidden rounded-2xl bg-gradient-to-br ${project.cover.theme} p-6 text-white`}>
    ...
    <div className="relative z-10 flex min-h-full w-full flex-col justify-between">
      ...
      <div className={compact ? 'mt-10' : 'mt-16'}>
        <h3 className={`${compact ? 'text-xl md:text-2xl' : 'text-3xl md:text-4xl'} thai-heading-safe max-w-[13ch] font-display font-extrabold`}>
          {project.cover.title}
        </h3>
        <p className="thai-safe mt-3 max-w-xs text-xs text-white/82">{project.cover.subtitle}</p>
      </div>
